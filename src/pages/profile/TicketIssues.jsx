...
const TicketIssues = ({ HomeNavigate, BackButton }) => {
  ...
  const headers = [
    columnConfig.ticketNo && {
      ...
      label: "Ticket No",
      render: (value) => (
        <span className="font-semibold text-[var(--color-primary,#4c2691)]">
          {value}
        </span>
      )
    },
    ...
    },
    {
      ...
      render: (_, row) => (
        <button
          className={`rounded-full w-8 h-8 p-0 inline-flex items-center justify-center text-white cursor-pointer border ${row.status === "closed" ? "bg-[#ff6b6b] border-[#ff6b6b]" : "bg-[var(--color-primary,#4c2691)] border-[var(--color-primary,#4c2691)] hover:bg-[var(--color-primary-dark,#5c33a6)] hover:border-[var(--color-primary-dark,#5c33a6)]"}`}
        ...
        >
          {selectedLead && (
            <div className="p-5 space-y-5 overflow-y-auto">
              {/* Ticket Information */}
              <div>
                <div style={{ fontSize: "10px", fontWeight: 700, color: "var(--color-primary,#4c2691)", textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: "12px" }}>
                  Ticket Information
                </div>
              ...
              {selectedLead.description && (
                <div>
                  <div style={{ fontSize: "10px", fontWeight: 700, color: "var(--color-primary,#4c2691)", textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: "8px" }}>
                    Description
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {selectedLead.attachments.map((attachment, index) => (
                      ...
                      >
                        ...
                        />
                        <div className="fallback-icon hidden w-full h-full items-center justify-center bg-slate-50 text-[var(--color-primary,#4c2691)]">
        ...
        <Offcanvas.Header closeButton onHide={closeChat}>
          <Offcanvas.Title>
            <div className="flex items-center gap-2">
              <div className="w-10 h-10 rounded-full bg-[var(--color-primary,#4c2691)] text-white flex items-center justify-center text-lg">
            ...
            {chatLoading ? (
              <>
                ...
                <div className="self-end max-w-[85%]">
                  <div className="bg-[var(--color-primary,#4c2691)]/40 rounded-[18px_18px_0_18px] h-5 w-[150px] animate-pulse" />
                </div>
                <div className="self-start max-w-[85%]">
                  ...
                  {chat.sender === "system" ? (
                    ...
                    <>
                      <div className={`flex items-end gap-2 ${chat.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                        {/* Icon */}
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-sm shrink-0 ${chat.sender === "user" ? "bg-[var(--color-primary,#4c2691)] text-white" : "bg-[#f0f2f5] text-slate-500"
                            }`}
                        >
                          {chat.sender === "user" ? (
                        ...
                        <div
                          className={`px-4 py-2.5 text-sm leading-relaxed max-w-[200px] shadow-[0_2px_5px_rgba(0,0,0,0.05)] ${chat.sender === "user"
                            ? "bg-[var(--color-primary,#4c2691)] text-white rounded-[18px_18px_0_18px]"
            ...
            {(selectedTicketForChat?.status?.toLowerCase() === "closed" ||
              ...
              <form onSubmit={handleSendMessage} className="flex gap-2">
                <input
                  ...
                  className="flex-grow h-[45px] rounded-[22px] border border-[#e0e0e0] px-5 text-sm outline-none focus:border-[var(--color-primary,#4c2691)]"
                />
                <button
                  type="submit"
                  className="w-[45px] h-[45px] rounded-full bg-[var(--color-primary,#4c2691)] text-white border-none flex items-center justify-center cursor-pointer transition-transform shadow-[0_4px_10px_rgba(128,89,202,0.3)] hover:scale-105"
                >
