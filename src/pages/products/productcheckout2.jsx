...
import Home2Header from "../../components/layout/Header-k.jsx";
import Footer from "../../components/layout/Footer-f.jsx";
...
import { navigateToLogin } from "../../utils/redirectUtils.js";
...
} from "../../components/ui/referredDoctorSelectUtils.jsx";
import { fetchDoctorsList } from "../../services/doctorService.js";
import { fetchFamilyMembersList } from "../../services/familyMemberService.js";
import BaseModal from "../../components/ui/BaseModal.jsx";
import PrescriptionUploadModal from "../../components/modals/PrescriptionUploadModal2.jsx";
...
export const Cart = () => {
  ...
  useEffect(() => {
    if (pendingOrderSubmit && !isSubmitting && cartItems.length > 0) {
      ...
      handleSubmit(null, verifiedPrescriptionImage);
    }
  ...
  const handlePrescriptionValidated = async (prescriptionImageVal, checkedMedicines, isTeleconsult = false) => {
    ...
    try {
      ...
      toast.error("Failed to update prescription details. Please try again.");
    } finally {
      ...
      try {
        ...
        if (response.data?.success && response.data?.data?.prescription_charge) {
          ...
          if (pc && pc.chargeAmount !== undefined) {
            setPrescriptionCharge(Number(pc.chargeAmount));
          }
        }
      } catch (e) {
  ...
  const handleSubmit = async (e, forcePrescriptionImage = null) => {
    ...
    const rxItems = cartItems.filter(item => {
      ...
      return rxRequired && !hasPrescription;
    });

    if (rxItems.length > 0 && !forcePrescriptionImage) {
      const medicineList = rxItems.map(item => item?.productDetails?.tabletDetails || item?.tabletdetails || item);
      setPrescriptionMedicine(medicineList);
  ...
  return (
    <div className="main-wrapper">
      ...
      >
        <div
          className={`card shadow-sm rounded-sm border-0 bg-white relative ${cartItems.length === 0 ? "w-full" : isMobile || isTablet ? "w-full" : "w-[67%]"} ${isMobile ? "p-4 mb-5" : "p-6 mb-0"}`}
        >
          ...
          <div
            className={
              isMobile
                ...
                : "bg-white rounded-sm border border-[#e2e8f0] shadow-[0_4px_20px_rgba(0,0,0,0.02)] p-6 overflow-x-auto [-webkit-overflow-scrolling:touch]"
            }
          >
            <div
        ...
        {cartItems.length > 0 && (
          <div
            className={`card shadow-sm rounded-sm border border-[#f1f5f9] bg-white shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05)]  ${isMobile || isTablet ? "w-full static" : "w-[33%] sticky"} ${isMobile ? "p-3" : "p-7"}`}
          >
            <div>
              {/* OFFERS */}
              <div className="mb-7">
      ...
      <PrescriptionUploadModal
        ...
        onValidated={handlePrescriptionValidated}
        medicineData={prescriptionMedicine}
        mode="analyze"
        pincode={currentLocation?.pincode || selectedAddress?.location?.pincode || selectedAddress?.pincode}
        lat={latitude}
        lng={longitude}
