...
const ProductsData = () => {
  ...
  const isTablet = useMediaQuery({ maxWidth: 1012 });
  ...
  const getCategoryAndBrandData = async () => {
    try {
      const response = await axiosCommonInstance.get(`category/search/filter/?serviceType=${service}&maincatId=${maincategories || id || null}&asortby=a`);
      ...
      setMedicineForms(response.data.data.medicineform || []);
      setCompositions(response.data.data.compositions || []);
      setAlphabetData(response.data.data.alpha || []);
      setComplexity(response.data.data.complexity || []);
      setGender(response.data.data.gender || []);
      setSamples(response.data.data.sampletypes || []);
      setConditions(response.data.data.conditions || []);
    } catch (error) {
      setCategories([]);
      setBrands([]);
  ...
  const handleClearFilters = () => {
    ...
    if (defaultCategoryToPreserve) {
      params.set("id", defaultCategoryToPreserve);
    }
    ...
    // Update URL params - always preserve default category
    if (defaultCategoryToPreserve) {
      searchParams.set("categories", defaultCategoryToPreserve);
    } else {
      searchParams.delete("categories");
    }
  ...
  const handleBooking = async (vendor, tablet) => {
    if (!token) {
  ...
  useEffect(() => {
    // Reset state when id or service changes
  ...
  }, [selectedCategories, selectedBrands, selectedTypes, selectedForms, selectedCompositions, selectedComplexity, selectedGender, selectedSamples, selectedConditions, page, userId, activeTab, id, service, isAlphabetFilterActive, selectedPincode, searchTerm]);
          ...
          <div className="row">
            ...
            >
              ...
              <ProductsSection
                ...
                onVendorAction={(action, vendor, tablet) => {
                  if (action === "lead") handleAddLead(vendor, tablet);
                  else if (action === "booking") handleBooking(vendor, tablet);
